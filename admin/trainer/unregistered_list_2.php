...
$table_data=(function(int $current_page,int $per_page)use($pdo){
    $parts[]="SELECT DISTINCT `rr`.trainer_unique_name";
    $parts[]="FROM `".RaceResults::TABLE."` as `rr`";
    $parts[]="LEFT JOIN `".Trainer::TABLE."` AS `t` ON `rr`.trainer_unique_name LIKE `t`.unique_name";
    $parts[]="WHERE `t`.unique_name IS NULL AND `rr`.trainer_unique_name NOT LIKE ''";
    $parts[]="ORDER BY `rr`.trainer_unique_name ASC";
    if($per_page>0){
