...
$table_data=(function(int $current_page,int $per_page)use($pdo){
    $parts[]="SELECT DISTINCT r.race_course_name";
    $parts[]="FROM `".RaceResults::TABLE."` as r";
    $parts[]="LEFT JOIN `".RaceCourse::TABLE."` AS c ON r.race_course_name LIKE c.unique_name";
    $parts[]="WHERE c.unique_name IS NULL AND r.race_course_name NOT LIKE ''";
    $parts[]="ORDER BY r.race_course_name ASC";
    if($per_page>0){
...
<p>レース結果だけに存在し、競馬場マスタにない競馬場名の一覧です。</p>
