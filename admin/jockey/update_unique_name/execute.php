...
$page->title="騎手マスタ管理｜キー名称の一括変更：実行";
$page->ForceNoindex();
$session=new Session();
...
$new_id_exists=$updater->new_id_exists(Jockey::TABLE,'unique_name');
if(!$new_id_exists){
    // UNIQUE制約があるため、存在しない場合のみunique_nameを変更
    $updater->addUpdateTarget(Jockey::TABLE,'unique_name');
}
$updater->addUpdateTarget(RaceResults::TABLE,'jockey_unique_name');
$updater->execute();
?><!DOCTYPE html>
<html>
<head>
