...
$race_category_table=new RaceCategorySex();
$race_category = $race_category_table->getPage($pdo,$search_page,true);
?><!DOCTYPE html>
<html lang="ja">
<head>
...
$first_tag =new MkTagA("[最初]",($race_category_table->current_page>2?('?'.$url_param->toString(['page'=>1])):''));
$prev_tag  =new MkTagA("[前へ]",($race_category_table->current_page>1?('?'.$url_param->toString(['page'=>$race_category_table->current_page-1])):''));
$next_tag  =new MkTagA("[次へ]",($race_category_table->has_next_page?('?'.$url_param->toString(['page'=>$race_category_table->current_page+1])):''));
?>
<!--
...
<?php foreach($race_category as $row): ?>
