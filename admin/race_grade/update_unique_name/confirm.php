...
$page->title="レース格付マスタ｜キー名称の一括変更：確認";
$page->ForceNoindex();
$session=new Session();
...
do{
    ...
    if(mb_strlen($new_unique_name)>5){
        $page->addErrorMsg('キー名は5文字以下で入力してください');
    }
...
<?php $new_id_exists=$updater->new_id_exists(RaceGrade::TABLE,'unique_name'); ?>
<?php if($new_id_exists): ?>
新名称[<?=h($new_unique_name)?>]のグレードは既にマスタに存在します。<br>
マスタの名称は変更せず、旧名称に該当するレースを新名称のレースに紐づけます。
<?php endif; ?>
