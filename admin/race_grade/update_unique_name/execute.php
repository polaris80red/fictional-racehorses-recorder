...
$page->title="レース格付マスタ｜キー名称の一括変更：実行";
$session=new Session();
...
$new_id_exists=$updater->new_id_exists(RaceGrade::TABLE,'unique_name');
if(!$new_id_exists){
    // UNIQUE制約があるため、存在しない場合のみunique_nameを変更
    $updater->addUpdateTarget(RaceGrade::TABLE,'unique_name');
}
$updater->addUpdateTarget(RaceResults::TABLE,'grade');
$updater->execute();
?><!DOCTYPE html>
<html>
<head>
...
<hr>
<a href="../list.php">レース格付マスタ一覧に移動</a>
</form>
<hr class="no-css-fallback">
</main>
<footer>
