...
class Setting{
     ...
     * 競走馬戦績での日付列の取得
     */
    public function getHorseRaceHistoryDate($year,$month,$day,$turn=0,int|null $age=null){
        $parts=new stdClass;
        $parts->year_age='';
        $ym_separator=$this->year_month_separator;
        if($this->horse_record_year===self::HORSE_RECORD_YEAR__AGE){
            // 馬齢モードの場合
            $parts->year_age=$this->getConvertedAge($age);
            $ym_separator=" ";
        }else{
            $parts->year_age=$this->getYearSpecialFormat($year);
        }
        if($this->horse_record_date==='y'){
            $ret=$parts->year_age;
            // 年のみモードはここで終了
            if($this->horse_record_year===self::HORSE_RECORD_YEAR__YEAR){
                $ret.=$this->date_format_part['year_suffix'];
            }
            return $ret;
        }
        $parts->month=str_pad($month, 2, "0", STR_PAD_LEFT);
        if($this->horse_record_date==='umm'){
            if(
                $this->year_view_mode===self::YEAR_VIEW_MODE_DEFAULT &&
                $this->horse_record_year===self::HORSE_RECORD_YEAR__YEAR){
                // 年の形式が標準で、なおかつ馬齢モードでない
                //$str.='年';
            }
            ...
            if($this->horse_record_year===self::HORSE_RECORD_YEAR__AGE){
                $key='amt';
                $ret.=$this->date_format_part[$key]['ya_m_separator'];
            }else{
                $key='ymt';
                $ret.=$this->date_format_part[$key]['ya_suffix'];
                $ret.=$this->date_format_part[$key]['ya_m_separator'];
            }
            $ret.=$parts->month;
            $ret.=$this->date_format_part[$key]['m_suffix'];
            $parts->turn='';
            if($turn===1){
                $parts->turn="前半";
            }else if($turn===2){
            ...
            if($parts->turn!==''){
                $ret.=$this->date_format_part[$key]['m_dt_separator'].$parts->turn;
            }   
        }else{
            $ret =$parts->year_age;
            if($this->horse_record_year===self::HORSE_RECORD_YEAR__AGE){
                if($this->horse_record_date==='ymd'){ $key='amd'; }
                if($this->horse_record_date==='ym'){ $key='am'; }
            }else{
                if($this->horse_record_date==='ymd'){ $key='ymd'; }
                if($this->horse_record_date==='ym'){ $key='ym'; }
            }
            $ret.=$this->date_format_part[$key]['ya_m_separator'];
            $ret.=$parts->month;
            $ret.=$this->date_format_part[$key]['m_suffix']??'';
            if($this->horse_record_date==='ym' && $ym_separator===" "){
                // 年月モードの場合、年月の間がスラッシュ区切りでなければ「月」を追加
                //$str.="月";
            }
            if($day>0 && $this->horse_record_date==='ymd'){
                $ret.=$this->date_format_part['ymd']['m_dt_separator']??'';
                $ret.=str_pad($day, 2, "0", STR_PAD_LEFT);
                $ret.=$this->date_format_part['ymd']['d_suffix']??'';
            }
        }
        return $ret;
    }
